...
with resources.as_file(PKG / "traditional.txt") as p, p.open(
    encoding="utf-8"
) as f, resources.as_file(PKG / "simplified.txt") as q, q.open(encoding="utf-8") as g:
    traditional, simplified = f.read().strip(), g.read().strip()

with resources.as_file(PKG / "bigram.json") as p, p.open(
    encoding="utf-8"
) as f, resources.as_file(PKG / "monogram.json") as q, q.open(encoding="utf-8") as g:
...
def default_getter(l, i, default: Any | None = None):
    try:
        return l[i]
    except IndexError:
        return default
...
def most_common_word(
    choices: list[str], prev_word: str | None = None, next_word: str | None = None
) -> str:
    ...
    for i, s in enumerate(text):
        try:
            ...
            if len(choices) > 1:
                result.append(
                    most_common_word(
                        choices, default_getter(result, -1), default_getter(text, i + 1)
                    )
                )
            else:
